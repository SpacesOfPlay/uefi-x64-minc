...
//
// After ExitBootServices the CPU still runs on the firmware's descriptor
// tables. Installing our own kernel code and data segments removes that last
// dependency and gives the IDT a known code selector to run handlers under.
// Ring 0 only — no user segments, no TSS.
...
// `gran` the granularity/flags byte.
